 ...
 *   User.findOrCreate({amazonId: profile.id}, function(error, user) {
 *     next(error, user);
 *   })
 * })
 */

var AmazonTokenStrategy = function (_OAuth2Strategy) {
  _inherits(AmazonTokenStrategy, _OAuth2Strategy);

  function AmazonTokenStrategy(_options, _verify) {
    _classCallCheck(this, AmazonTokenStrategy);
    ...
    var _this = _possibleConstructorReturn(this, Object.getPrototypeOf(AmazonTokenStrategy).call(this, options, verify));

    _this.name = 'amazon-token';
    ...
    _this._profileURL = options.profileURL || 'https://api.amazon.com/user/profile';
  ...
  _createClass(AmazonTokenStrategy, [{
    key: 'authenticate',
      ...
      this._oauth2.get(this._profileURL, accessToken, function (error, body, res) {
        if (error) {
          try {
            ...
            return done(new _passportOauth.InternalOAuthError(errorJSON.error_description, errorJSON.error));
          } catch (_) {
  ...
  return AmazonTokenStrategy;
}(_passportOauth.OAuth2Strategy);

exports.default = AmazonTokenStrategy;
;
